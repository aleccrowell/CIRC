...
def _make_prot_result(
    rseed=0, with_uncertainty=False, n_proteins=50, peptides_per_protein=3
):
    """Proteomics-style result with a (Peptide, Protein) MultiIndex."""
    rng = np.random.default_rng(rseed)
    n = n_proteins * peptides_per_protein
    proteins = [
        f"PROT_{p:04d}" for p in range(n_proteins) for _ in range(peptides_per_protein)
    ]
...
class TestCompareConditions:
    ...
    def test_delta_pirs_correct(self):
        ...
        shared = A.index.intersection(B.index)
        expected = (
            B.loc[shared, "pirs_score"].values - A.loc[shared, "pirs_score"].values
        )
...
class TestAggregateToProtein:
    ...
    def test_aggregate_then_compare(self):
        ...
        prot = aggregate_to_protein(
            _make_prot_result(rseed=0, n_proteins=50, peptides_per_protein=2)
        )
